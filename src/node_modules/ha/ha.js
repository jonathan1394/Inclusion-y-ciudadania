...
var CHILD_TIMEOUT = 10000; // ms before the program exits due to a child command
                           // not completing
var ARP_INTERVAL = 60000; // reassert the cluster address every minute
...
// This is needed in case a slave becomes master, due to network partition, and breifly takes
// over the interface.  If this the address was not reasserted, netwrok traffice would continue
// to go to the slave, as the slave has no way of "unarping" the address.
setInterval(function() {
  if (!isActive()) return;
  var shared_addr = findAddress(SHARED_ETHERNET_DEVICE, IP_VERSION);
  if (!shared_addr) { 
    console.warn('shared address not found');
    return;
  }
...
function change_state(new_state) {
  ...
  if (state === STANDBY && new_state === ACTIVE) {
    ...
    spawn2("ifup", [SHARED_ETHERNET_DEVICE], function(err) {
      if (err) { // report any error
        console.warn('ifup', SHARED_ETHERNET_DEVICE, 'failed. err:', err);
      }
    });
  }
  if (new_state !== state) {
